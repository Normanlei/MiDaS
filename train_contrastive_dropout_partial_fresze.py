...
class NYUDepthV2Dataset(Dataset):
    def __init__(self, image_dir, ground_truth_base=None, transform=None):
        ...
        for scene_path in scene_dirs:
            ...
            augmented_input_dir = input_dir.replace(
                "official_splits", "official_splits_augmented"
            )
            input_images = sorted(glob.glob(os.path.join(input_dir, "*")))
            augmented_input_images = sorted(
                glob.glob(os.path.join(augmented_input_dir, "*"))
            )
            depth_maps = sorted(glob.glob(os.path.join(gt_dir, "*.pfm")))
            # print(input_dir, augmented_input_dir, gt_dir)
            if (
                len(input_images) != len(depth_maps)
                or len(augmented_input_images) != len(depth_maps)
                or len(input_images) != len(augmented_input_images)
            ):
                raise ValueError(
                    f"Image/Augmented/depth count mismatch with {len(input_images)} vs {len(augmented_input_images)} vs {len(depth_maps)}"
                )
...
if __name__ == "__main__":
    ...
    dataset = NYUDepthV2Dataset(
        image_dir="../../data/official_splits/train", transform=transform
    )
    ...
    for name, param in model.pretrained.named_parameters():
        print(name)
        if name.startswith("model.patch_embed") or \
        name.startswith("model.cls_token") or \
        name.startswith("model.pos_embed") or \
        any(name.startswith(f"model.blocks.{i}") for i in range(8)):  # blocks 0 to 7
            param.requires_grad = False
    ...
    for epoch in range(EPOCHS):
        ...
        for img, augmented_img, depth in tqdm(train_loader):
            img, augmented_img, depth = (
                img.to(device),
                augmented_img.to(device),
                depth.to(device),
            )
            ...
            augmented_prediction = torch.nn.functional.interpolate(
                augmented_prediction.unsqueeze(1),
                size=target_size,
                mode="bicubic",
                align_corners=False,
            ).squeeze()
            ...
            prediction_aligned = scale1.view(-1, 1, 1) * prediction + shift1.view(
                -1, 1, 1
            )
            scale2, shift2 = compute_scale_and_shift(augmented_prediction, depth, mask)
            augmented_prediction_aligned = scale2.view(
                -1, 1, 1
            ) * augmented_prediction + shift2.view(-1, 1, 1)

            loss_consistency = F.l1_loss(
                prediction_aligned, augmented_prediction_aligned
            )

            loss = (
                loss_fn(prediction, depth, mask)
                + REG_CONSISTENCY_TERM * loss_consistency
            )
        ...
        with torch.no_grad():
            for img, augmented_img, depth in tqdm(val_loader):
                ...
                mask = depth > 0
                scale1_val, shift1_val = compute_scale_and_shift(
                    prediction, depth, mask
                )
                prediction_aligned = scale1_val.view(
                    -1, 1, 1
                ) * prediction + shift1_val.view(-1, 1, 1)
                scale2_val, shift2_val = compute_scale_and_shift(
                    augmented_prediction, depth, mask
                )
                augmented_prediction_aligned = scale2_val.view(
                    -1, 1, 1
                ) * augmented_prediction + shift2_val.view(-1, 1, 1)
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save(
                    model.state_dict(),
                    f"freeze0-7dropout0.3_contrast_best_model_epoch{epoch+1}.pt",
                )
                print(
                    f"✅ Best model updated at Epoch {epoch+1} with Val Loss: {best_val_loss:.4f}"
                )
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"freeze0-7dropout0.5_contrast_loss_curve.png")
    plt.close()
